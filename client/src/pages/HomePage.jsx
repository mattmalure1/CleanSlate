...
export default function HomePage() {
  ...
  return (
    <div>
      ...
      {/* ─── PRICING RANGE (no anchoring — scan to find out) ─── */}
      <section className="bg-brand-50/40 border-y border-brand-100">
        <div className="max-w-3xl mx-auto px-[var(--spacing-page)] py-14 text-center">
          ...
          <h2 className="font-display text-2xl sm:text-3xl font-bold text-text-primary mt-2">
            Wondering what your stuff is worth?
          </h2>
          <p className="mt-4 text-base sm:text-lg text-text-secondary leading-relaxed">
            Offers range from <span className="font-semibold text-text-primary">$0.10 per item</span> for common titles up to <span className="font-semibold text-text-primary">$100+</span> for premium textbooks, modern video games, and rare collector pieces. The exact amount depends on resale value — scan an item and we'll show you the offer in seconds.
          </p>
          <button
            onClick={scrollToSearch}
            className="mt-6 inline-flex items-center gap-2 bg-brand-600 hover:bg-brand-700 active:scale-[0.97] text-white font-semibold text-sm px-6 py-3 rounded-xl shadow-sm cursor-pointer min-h-[48px]"
          >
            <ScanBarcode size={18} />
